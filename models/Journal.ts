...
export interface IJournal extends Document<string> {
  ...
  entryDate: Date;
  referenceType: 'POSSale' | 'POSReturn' | 'Invoice' | 'Receipt' | 'Payment' | 'Purchase' | 'Expense' | 'PurchaseReturn' | 'SalesReturn' | 'General' | 'Contra' | 'Adjustment';
...
const JournalSchema: Schema<IJournal> = new Schema({
  ...
  referenceType: {
    type: String,
    enum: ['POSSale', 'POSReturn', 'Invoice', 'Receipt', 'Payment', 'Purchase', 'Expense', 'PurchaseReturn', 'SalesReturn', 'General', 'Contra', 'Adjustment'],
