...
const s = StyleSheet.create({
    ...
    summaryBox: {
        flex: 1, backgroundColor: pdfColors.secondary,
        borderLeftWidth: 3, borderLeftColor: pdfColors.primary,
        padding: spacing.sm,
    },
    ...
    totalRow: {
        ...
        borderTopWidth: 2, borderTopColor: pdfColors.primary, backgroundColor: pdfColors.secondary,
    },
...
export const SalesReportDocument: React.FC<SalesReportDocumentProps> = ({
    monthlyBreakdown, summary, dateRange, companyDetails,
}) => {
    registerPdfFonts();
    ...
    return (
        <Document>
            <Page size="A4" style={commonStyles.page}>
                ...
                <DocumentTitle title="Sales Report" />
                ...
                <View style={s.summaryGrid}>
                    <View style={s.summaryBox}>
                        ...
                        <Text style={s.summaryValue}>{formatCurrency(summary.totalRevenue)}</Text>
                    </View>
                    <View style={s.summaryBox}>
                        <Text style={s.summaryLabel}>Total Tax</Text>
                        <Text style={s.summaryValue}>{formatCurrency(summary.totalTax)}</Text>
                    </View>
                    <View style={s.summaryBox}>
                        <Text style={s.summaryLabel}>Total Invoices</Text>
                        <Text style={s.summaryValue}>{summary.totalInvoices}</Text>
                    </View>
                    <View style={s.summaryBox}>
                        <Text style={s.summaryLabel}>Avg Invoice Value</Text>
                        <Text style={s.summaryValue}>{formatCurrency(summary.avgInvoiceValue)}</Text>
                    </View>
                </View>
                ...
                <View style={s.tableHeader}>
                    ...
                    <Text style={[s.th, s.colRevenue]}>Revenue</Text>
                ...
                {monthlyBreakdown.map((row, i) => (
                    <View key={i} style={[s.tableRow, i % 2 === 1 ? s.tableRowAlt : {}]}>
                        ...
                        <Text style={[s.td, s.colCount]}>{row.invoiceCount}</Text>
                        <Text style={[s.td, s.colRevenue]}>{formatCurrency(row.revenue)}</Text>
                ...
                <View style={s.totalRow}>
                    ...
                    <Text style={[s.totalValue, s.colCount]}>{summary.totalInvoices}</Text>
                    <Text style={[s.totalValue, s.colRevenue]}>{formatCurrency(summary.totalRevenue)}</Text>
