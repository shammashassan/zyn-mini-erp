// app/sales/credit-notes/credit-note-form.tsx
...
export function CreditNoteForm({ isOpen, onClose, onSubmit, defaultValues, returnNoteData }: CreditNoteFormProps) {
  ...
  const vatAmount = creditMode === 'items'
    ? watchedItems.reduce((sum, item) => sum + (Number(item.taxAmount) || 0), 0)
    : 0;
  const grandTotal = subtotal + vatAmount;
  const totalItems = creditMode === 'items' ? watchedItems.filter(item => item.description).length : 1;
  ...
  // Fetch approved sales return notes based on partyId
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (returnNoteData) {
      ...
      const itemsFromReturn = returnNoteData.items.map((item: any): CreditNoteItem => ({
        itemId: item.itemId || '',
      ...
      setSelectedPartyType(partyRoles?.supplier ? 'supplier' : 'customer');
  ...
  const handleReturnNoteSelect = (returnNote: any) => {
    ...
    const itemsFromReturn: CreditNoteItem[] = returnNote.items.map((item: any) => ({
      itemId: item.itemId || '',
      description: item.description || '',
      quantity: item.returnQuantity || 0,
      price: item.rate || 0,
  ...
  const handleFormSubmit = async (data: CreditNoteFormData) => {
    ...
    if (!data.creditDate) { toast.error("Please select a credit note date"); return; }
    if (!data.reason?.trim()) { toast.error("Please provide a reason for this credit note"); return; }
    ...
    const calculatedVatAmount = data.creditMode === 'items'
      ? validItems.reduce((sum, item) => sum + (Number(item.taxAmount) || 0), 0)
      : 0;
    ...
    if (data.returnNoteId) {
      ...
      if (rn) {
        ...
        submitData.returnNumber = rn.returnNumber;
      ...
      <DialogContent className="max-w-[95vw] lg:max-w-3xl max-h-[90vh] overflow-y-auto sidebar-scroll">
        ...
        <form onSubmit={(e) => { e.stopPropagation(); handleSubmit(handleFormSubmit)(e); }} className="space-y-6">
            ...
            <div className="p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-900">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Items and quantities are pre-filled. You can adjust prices if needed.</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <Textarea id="reason" placeholder="Enter reason for credit note..." {...register("reason")} rows={2} />
          </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <CardTitle className="text-base">{creditMode === 'items' ? 'Items' : 'Amount Details'}</CardTitle>
              {!isFromReturnNote && (
                <Controller name="creditMode" control={control}
                  render={({ field }) => (
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting || (isEditMode && !isDirty)}>
              {isSubmitting ? (<><Spinner />Saving...</>) : (defaultValues ? "Update Credit Note" : "Create Credit Note")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
