...
import { TaxReportChart } from "./tax-chart";
import { StatsCards, type StatItem } from "@/components/shared/stats-cards";
import { getTaxColumns, type TaxReportData } from "./columns";
...
import { exportTaxReportToExcel } from "@/utils/reportExports";
...
function TaxReportPageContent() {
  ...
  const fetchData = useCallback(async (background = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/tax-report?${params}`);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/tax-report/pdf?${params}`);
  ...
  const handleExportExcel = () => {
    ...
    setIsExporting(true);
    try {
      exportTaxReportToExcel(data.monthlyBreakdown, data.summary, { from: dateRange!.from!, to: dateRange!.to! }, null);
      toast.success('Exported to Excel');
    } catch { toast.error('Failed to export Excel'); }
    finally { setIsExporting(false); }
  };

  const columns = useMemo(() => getTaxColumns(taxView), [taxView]);
  ...
  const cardsData: StatItem[] = useMemo(() => {
    ...
    const { summary } = data;
    const totalTaxActivity = summary.totalSalesTax + summary.totalPurchaseTax;

    return [
      {
        name: "Sales Tax",
        stat: formatCompactCurrency(summary.totalSalesTax),
        change: `${summary.salesTaxRate.toFixed(1)}% rate`,
        changeType: "positive",
        subtext: `${summary.salesTransactions} invoices`
      },
      {
        name: "Purchase Tax",
        stat: formatCompactCurrency(summary.totalPurchaseTax),
        change: `${summary.purchaseTaxRate.toFixed(1)}% rate`,
        changeType: "negative",
        subtext: `${summary.purchaseTransactions} purchases`
      },
      {
        name: "Net Tax Liability",
        stat: formatCompactCurrency(Math.abs(summary.netTaxLiability)),
        change: summary.netTaxLiability >= 0 ? "To pay" : "Refund due",
        changeType: summary.netTaxLiability >= 0 ? "negative" : "positive",
        subtext: "Sales tax - Purchase tax"
      },
      {
        name: "Total Tax",
        stat: formatCompactCurrency(totalTaxActivity),
        change: "Combined",
        changeType: "neutral",
        subtext: `${summary.salesTransactions + summary.purchaseTransactions} transactions`
      }
    ];
  }, [data]);
              ...
              <div className="flex items-center gap-3 self-start lg:self-center">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Sales tax collected and purchase tax paid.
                  </p>
                </div>
              </div>
              ...
              <Card>
                <CardHeader className="pb-4">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div>
                      <CardTitle>
                        Tax {taxView === "sales" ? "Sales" : "Purchase"} Report for {formatDateRange()}
                      </CardTitle>
                      <CardDescription>
                        {taxView === "sales"
                          ? "Monthly aggregated sales tax data from approved invoices (calculated from Journal entries)"
                          : "Monthly aggregated purchase tax data from received purchases (calculated from Journal entries)"
                        }
                      </CardDescription>
                    </div>

                    <Tabs value={taxView} onValueChange={(value: string) => setTaxView(value as "sales" | "purchase")} className="w-auto">
                      <TabsList className="grid w-full grid-cols-2">
                        ...
                        <TabsTrigger value="purchase" className="flex items-center gap-2">
                          ...
                          <span className="hidden sm:inline">Purchase Tax</span>
                        </TabsTrigger>
                      </TabsList>
                    </Tabs>
                  </div>
