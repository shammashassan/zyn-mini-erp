...
export function SalesChart({ data, totalSales, totalOrders, dateRange }: SalesChartProps) {
  const [viewType, setViewType] = React.useState<"sales" | "orders">("sales");
  ...
  const calculateTrend = () => {
    ...
    const firstHalfAvg = firstHalf.reduce((sum, item) =>
      sum + (viewType === "sales" ? item.sales : item.orders), 0) / firstHalf.length;
    const secondHalfAvg = secondHalf.reduce((sum, item) =>
      sum + (viewType === "sales" ? item.sales : item.orders), 0) / secondHalf.length;
  ...
  const currentMetric = viewType === "sales" ? totalSales : totalOrders;
      ...
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          ...
          <CardDescription>
            <span className="hidden @[540px]/chart:block">
              {formatDateRange()} • {viewType === "sales" ? "Sales amount" : "Number of orders"} trend over time
            </span>
        ...
        <div className="flex gap-2">
          <ToggleGroup
            type="single"
            value={viewType}
            onValueChange={(value) => value && setViewType(value as "sales" | "orders")}
            variant="outline"
          ...
          <Select value={viewType} onValueChange={(value: "sales" | "orders") => setViewType(value)}>
            <SelectTrigger
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-sm">
                      {viewType === "sales" ? "Total Sales" : "Total Orders"}
                    </span>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-sm">
                      {viewType === "sales" ? formatCompactCurrency(currentMetric) : currentMetric.toLocaleString()}
                    </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-medium">{formatCompactCurrency(totalSales)}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="flex items-center gap-2">
